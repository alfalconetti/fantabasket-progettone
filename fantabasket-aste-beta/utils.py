...
def get_fa_rows() -> list[dict]:
    """
    Restituisce la lista FA.
    In beta: da PostgreSQL (giocatori senza contratto attivo).
    Fallback: CSV (modalità isolata PG non disponibile).
    """
    import pg_client
    if pg_client.pg_disponibile():
        return pg_client.get_fa_rows_pg()
    # Fallback CSV
    rows = []
...
def segna_giocatore_firmato(nome: str):
    """
    In beta: no-op — la firma è gestita dalla transazione PG.
    Fallback CSV in modalità isolata.
    """
    import pg_client
    if pg_client.pg_disponibile():
        return  # il giocatore sparisce dalla FA list automaticamente via PG
    # Fallback CSV
    import shutil
    rows_csv = []
    with open(FA_CSV_PATH, newline="", encoding="utf-8") as f:
    ...
    with open(tmp, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for r in rows_csv:
            if normalizza(r["nome"].strip()) == normalizza(nome):
                r["firmato"] = "1"
            writer.writerow(r)
...
def build_canale_message(asta: dict, offerte: list, teams_map: dict, aperta_da: str = None) -> str:
    ...
    if stato == "APERTA":
        ...
        anni = int(asta["anni_offerti"]) if asta["anni_offerti"] else None
        anni_label = str(anni) if anni else "?"
        prop_nome = teams_map.get(asta["squadra_proprietaria"], "") if asta["squadra_proprietaria"] else ""
        footer = (
            f"{corrente} × {anni_label} ann{'o' if anni == 1 else 'i'}\n\n"
...
# ── helper cap/slot per display (PG-first) ────────────────────────────────────

def cap_slot_display(team: dict, stagione: str) -> tuple[int, int]:
    """
    Restituisce (cap_totale, slot_totali) per la visualizzazione.
