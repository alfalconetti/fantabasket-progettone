...
_token_file = os.environ.get("BOT_TOKEN_FILE")
TOKEN = open(_token_file).read().strip() if _token_file else os.environ["BOT_TOKEN"]
...
def main():
    ...
    async def _register_commands(app):
        ...
        cmd_gm = [
            ...
            BotCommand("offri",     "Lista aste aperte con bottone offerta"),
            BotCommand("offerta",   "Offerta diretta: /offerta asta <id> <importo>"),
            BotCommand("nuova_fa",  "Apri asta per un FA: /nuova_fa <giocatore>"),
            BotCommand("watch",     "Aggiungi un'asta alla watchlist"),
            BotCommand("watched",   "Aste che stai seguendo"),
