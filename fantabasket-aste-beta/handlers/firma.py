...
async def chiedi_anni(context: ContextTypes.DEFAULT_TYPE, asta_id: int, schedula_job: bool = True):
    ...
    testo, kb = _anni_kb_e_testo(asta)
...
def _anni_kb_e_testo(asta: dict) -> tuple:
    """Helper: costruisce keyboard e testo selezione anni, riutilizzato da chiedi_anni e firma_indietro_callback."""
    importo = asta["offerta_corrente"]
    is_rfa  = asta["tipo"] == "RFA"
    ore     = settings.timeout_firma_rfa_ore() if is_rfa else settings.timeout_firma_fa_ore()
    bottoni = []
    for a in [1, 2, 3]:
        if a >= anni_minimi(importo):
            bottoni.append(InlineKeyboardButton(
                ...
                callback_data=f"firma:{asta['id']}:{a}"
            ))
...
async def firma_confirm_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
async def firma_indietro_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
async def pareggio_anni_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    import pg_client
    from handlers.offerte import _cap_libero as _cap_lib_offerte
    cap_impegnato  = db.get_cap_virtuale(team["id"])
    slot_impegnati = db.get_slot_virtuali(team["id"])
    cap_libero     = _cap_lib_offerte(team["id"]) + vec_comp
    if pg_client.pg_disponibile():
        slot_liberi = settings.slot_massimo() - pg_client.get_roster_count(team["id"]) - slot_impegnati
    else:
        slot_liberi = tm.get_team_by_id(team["id"])["slot_disponibili"] - slot_impegnati
...
async def _registra_firma_finale(
    ...
    if asta["tipo"] == "FA":
        ...
        if asta["vecchio_compenso"]:
            tm.libera_cap(asta["squadra_proprietaria"], asta["vecchio_compenso"],
                          nome_giocatore=asta["giocatore"])
        # scala cap e slot a chi firma (il giocatore RFA non occupava slot)
        tm.scala_cap_slot(team_id, importo)

    # Scrivi firma su PostgreSQL
    import pg_client
    ok_pg = pg_client.registra_firma(team_id, asta["giocatore"], importo, anni, stagione)
    if not ok_pg:
