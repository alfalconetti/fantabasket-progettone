...
async def listteams(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    righe = ["<b>🏀 Squadre</b>\n"]
    stagione_ls = utils.load_globals().get("stagione_corrente", "2025")
    for t in all_teams:
        cap_virt  = db.get_cap_virtuale(t["id"])
        slot_virt = db.get_slot_virtuali(t["id"])
        cap_disp, slot_disp = utils.cap_slot_display(t, stagione_ls)
...
async def set_cap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    await update.effective_message.reply_text(
        "⚠️ In modalità beta cap e slot sono gestiti da PostgreSQL.\n"
        "Usa /set_cap_penalizzato per la penalità cap."
    )
...
async def set_slot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    await update.effective_message.reply_text(
        "⚠️ In modalità beta gli slot sono calcolati dal roster in PostgreSQL."
    )
...
async def set_fase(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Beta: cap derivato da PG, non aggiorniamo teams.json
    righe.append("<i>Beta: cap e slot derivano da PostgreSQL — nessuna modifica al JSON.</i>")
    righe.append(f"Fase impostata a <b>{fase}</b>.")
...
async def _check_cap_virtuale_negativo(context, team_id: str, team_nome: str):
    ...
    if not team_data:
        return
    stagione_cv = utils.load_globals().get("stagione_corrente", "2025")
    cap_disp, _ = utils.cap_slot_display(team_data, stagione_cv)
    if (cap_disp - cap_virt) < 0:
        await _log_warn(
            context,
            f"⚠️ <b>{team_nome}</b> ha cap virtuale negativo dopo modifica admin!\n"
            f"Cap disponibile: {cap_disp}M · Cap impegnato: {cap_virt}M"
        )
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"Differenza: {cap_disp - cap_virt}M"
                    ),
                    parse_mode="HTML",
                )
            except Exception as e:
...
async def annulla_offerta(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if preview["nuovo_team"]:
        ...
        if team_data:
            stagione_ao = utils.load_globals().get("stagione_corrente", "2025")
            cap_disp, _ = utils.cap_slot_display(team_data, stagione_ao)
    ...
    if result["nuovo_team"]:
        ...
        if team_data:
            stagione_ao2 = utils.load_globals().get("stagione_corrente", "2025")
            cap_disp_ao2, _ = utils.cap_slot_display(team_data, stagione_ao2)
            if (cap_disp_ao2 - cap_virt) < 0:
                warning_cap = (
                    f"\n\n⚠️ <b>Attenzione:</b> {teams_map.get(result['nuovo_team'], result['nuovo_team'])} "
                    f"ha cap virtuale negativo ({cap_disp_ao2}M disponibile, "
...
def _cap_virtuale_post_riapertura(asta: dict) -> tuple[int, int, int]:
    ...
    if not team_data:
        return 0, 0, 0
    stagione_ri = utils.load_globals().get("stagione_corrente", "2025")
    cap_disp, _ = utils.cap_slot_display(team_data, stagione_ri)
    cap_virt  = db.get_cap_virtuale(team_id)
...
async def cmd_backup_ora(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Invia immediatamente un backup al canale log. Solo admin."""
    if not is_admin(update.effective_user.id):
        await update.effective_message.reply_text("⛔ Non sei autorizzato.")
        return
    ...
    if not log_channel_id:
        await update.effective_message.reply_text("❌ log_channel_id non configurato in globals.json.")
        return
    try:
        await invia_backup(context, log_channel_id, "manuale")
        await update.effective_message.reply_text("✅ Backup inviato al canale log.")
    except Exception as e:
        await update.effective_message.reply_text(f"❌ Backup fallito: <code>{e}</code>", parse_mode="HTML")
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    testo = (
        ...
        "/backup_ora — invia subito un backup al canale log\n"
...
def get_handlers():
    return [
        ...
        CommandHandler("backup_ora",            cmd_backup_ora),
