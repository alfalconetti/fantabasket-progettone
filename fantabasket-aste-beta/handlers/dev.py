...
async def dev_cap(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    import utils as _utils_dc
    stagione_dc = _utils_dc.load_globals().get("stagione_corrente", "2025")
    righe = ["💰 <b>Cap e slot virtuali</b>", ""]

    for team in sorted(tutti_team, key=lambda t: t["nome"]):
        tid = team["id"]
        cap_tot, slot_tot = _utils_dc.cap_slot_display(team, stagione_dc)
...
async def dev_version(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        BOT_VERSION = getattr(_bot, "BOT_VERSION", "sconosciuta")
    except Exception:
